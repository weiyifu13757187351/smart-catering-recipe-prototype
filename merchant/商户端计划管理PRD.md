...
> 12. **新增每锅投料量（生产计划）**：`W ÷ 锅数`（均分），任务表加一列、调整抽屉加总览格与「本任务用料」逐行值。
>     由 `锅数 = max ⌈W ÷ C⌉` 可证每锅投料恒 ≤ 最小占用设备的额定容量。
...
6. 锅数取值：≥ 1 的整数；不允许为 0；
7. **每锅投料量 = `W ÷ 锅数`（均分）** **【已确认】**：
   - 由 `锅数 = max ⌈W ÷ C⌉ ≥ ⌈W ÷ C_min⌉` 可证 **每锅投料量恒 ≤ 最小占用设备的额定容量**，不会装不下；
   - 逐行的每锅投料 = 该行本次用量 ÷ 锅数；
   - 生产人员要的是「这一锅下多少料」，不是「最后一锅剩多少」，所以给**均分**值作为装载指令；
...
**每锅投料 `170.45 ÷ 15 = 11.36kg`**。该菜另有「预热炒锅」步骤（同一设备，单锅 60 秒）与「酱汁投放」步骤（单锅 20 秒），则：
