(function(){
  ...
  // 标签与字典失效（被停用或删除）后：新建/编辑时不可再选择，受影响的菜谱在编辑时提示重新选择，提交校验会提示存在未完成的必填内容。
  let invalidSeed=[];
  const dictLabel={cooking:'烹饪分类',province:'菜谱省份',cuisine:'菜系分类'};
  const seedInvalid=d=>{const st=window.recipeDictState;if(!st||!d)return [];const out=[];const tags=[...(d.tags||[])].flatMap(g=>g[1]);tags.forEach(n=>{if(st.isRemoved('tags',n))out.push(['tags',n,'菜谱标签'])});['cooking','province','cuisine'].forEach(t=>{if(d[t]&&st.isRemoved(t,d[t]))out.push([t,d[t],dictLabel[t]])});return out};
  const dictValue=(type,value)=>{const st=window.recipeDictState;return st&&st.isRemoved(type,value)?'':value};
  const dictSelect=(type,list,value)=>{const st=window.recipeDictState,items=st?st.options(type,list):list;return (!value||!items.includes(value)?'<option value="">请选择</option>':'')+items.map(x=>`<option ${x===value?'selected':''}>${x}</option>`).join('')};
  const invalidNotice=()=>{const st=window.recipeDictState;if(!st||!invalidSeed.length)return '';const blocked=invalidSeed.filter(x=>x[0]==='tags'?!base.tags.some(n=>!st.isRemoved('tags',n)):!base[x[0]]);return `<div class="dict-invalid-note">该菜谱原使用的 ${invalidSeed.map(x=>`${x[2]}「${x[1]}」`).join('、')} 已被停用或删除，请重新选择有效内容。${blocked.length?'未完善前提交会提示「存在未完成的必填内容」。':''}</div>`};
  ...
  function basicTab(){const count=Object.values(base.foreign||{}).filter(Boolean).length,cats=categoryOptions(),tagGroupList=window.recipeDictState?window.recipeDictState.tagGroups(labelGroups):labelGroups,seconds=(cats.find(x=>x.name===base.categoryLevel1)?.children)||cats[0]?.children||[];return `<section class="device-basic-section">${invalidNotice()}<div class="device-basic-grid-v2"><div class="device-image-field"><span><i>*</i> 图片</span><label class="device-image-upload ${base.image?'has-image':''}">${base.image?`<img src="${base.image}" alt="菜谱图片">`:'<b>＋</b><small>上传1张图片</small>'}<input id="recipeBaseImage" type="file" accept="image/png,image/jpeg,image/webp"></label></div><div class="device-basic-fields"><label><i>*</i> 菜谱名称<input data-recipe-base="name" value="${esc2(base.name)}" placeholder="请输入菜谱名称"></label><label><i>*</i> 别名<input data-recipe-base="alias" value="${esc2(base.alias)}" placeholder="请输入菜谱别名"></label><label>外语菜谱名<button type="button" class="language-config-button" id="openRecipeLanguages">已配语种 ${count}/10 <em>设置 ›</em></button></label><label><i>*</i> 烹饪分类<select data-recipe-base="cooking">${dictSelect('cooking',['炒','炖','蒸','煮','烤'],base.cooking)}</select></label><label><i>*</i> 菜谱省份<select data-recipe-base="province">${dictSelect('province',['全国','四川','山东','广东','江苏','浙江'],base.province)}</select></label><label><i>*</i> 出餐成品系数<div class="percent-input"><input data-recipe-base="yieldRate" type="number" min="0.01" max="999.99" step="0.01" value="${esc2(base.yieldRate)}"><span>%</span></div></label><label><i>*</i> 菜谱分类<div class="recipe-category-cascade"><select data-recipe-base="categoryLevel1" id="recipeCategoryLevel1">${cats.map(x=>`<option ${x.name===base.categoryLevel1?'selected':''}>${x.name}</option>`).join('')}</select><span>›</span><select data-recipe-base="categoryLevel2" id="recipeCategoryLevel2">${seconds.map(x=>`<option ${x===base.categoryLevel2?'selected':''}>${x}</option>`).join('')}</select></div><small>来自分类管理，请先选择一级分类，再选择二级分类</small></label><label><i>*</i> 菜系分类<select data-recipe-base="cuisine">${dictSelect('cuisine',['川菜','鲁菜','粤菜','苏菜','浙菜'],base.cuisine)}</select></label><label class="full">菜品简介<textarea data-recipe-base="intro" placeholder="请输入菜品简介">${esc2(base.intro)}</textarea></label></div></div><div class="base-tag-picker"><h3><i>*</i> 菜品标签 <small>可多选，只能选择二级标签</small></h3>${tagGroupList.map(([group,items])=>`<section><h4>${group}</h4><div>${items.map(x=>`<label><input type="checkbox" data-recipe-tag value="${x}" ${base.tags.includes(x)?'checked':''}> ${x}</label>`).join('')}</div></section>`).join('')}</div></section>`}
