(()=>{
  ...
  function accountDetail(account){const role=roleById(account.roleId);shell('运营账号详情',`${account.name} · ${account.mobile}`,`<div class="sys-section"><h3>账号信息</h3><div class="sys-detail-grid"><div><span>姓名</span><b>${esc(account.name)}</b></div><div><span>登录手机号</span><b>${account.mobile}</b></div><div><span>账号状态</span><b>${account.status}</b></div><div><span>所属角色</span><b>${esc(role?.name||'—')}</b></div><div><span>数据权限</span><b>${role?.scope==='all'?'全部数据':'仅本人创建'}</b></div>${account.reminder==='待提醒'?'<div><span>首次登录提醒</span><b>待提醒</b></div>':''}<div><span>最后登录</span><b>${account.lastLogin||'从未登录'}</b></div><div><span>创建信息</span><b>${account.creator} · ${account.created}</b></div></div></div><div class="sys-section"><h3>功能权限</h3><div class="sys-tag-list">${(role?.permissions||[]).map(p=>`<span>${esc(p)}</span>`).join('')}</div></div><div class="first-login-note"><b>ⓘ</b><span>账号权限全部继承自单一角色。数据权限为“仅本人创建”时，同时限制查看、编辑、删除、发布、撤销和导出。</span></div>`)}
